...
class Round:
    ...
    def to_dict(self):
        ...
        str_start_time = self.start_time.strftime("%m/%d/%Y, %H:%M:%S") if self.start_time else None
    ...
    def from_dict(cls, store, game_dict):
        ...
        for r in game_dict:
            round = Round(r['name'])
            round.start_time = datetime.strptime(r['start_time'], '%m/%d/%Y, %H:%M:%S') if r['start_time'] else None
            round.ended_time = datetime.strptime(r['ended_time'], '%m/%d/%Y, %H:%M:%S') if r['ended_time'] else None
